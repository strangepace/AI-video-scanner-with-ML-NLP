...
class Config:
    ...
    PREVIEW_LENGTH = 30  # frames per preview
    BATCH_SIZE = 8
...
class VideoProcessor:
    ...
    def process_frame(self, frame: np.ndarray, prompt: str) -> List[Dict[str, Any]]:
        """Process a single frame and return detections that match the prompt"""
        try:
            ...
            for box in results.boxes:
                try:
                    ...
                    with torch.no_grad():
                        ...
                        similarity = float(F.softmax(outputs.logits_per_image, dim=1)[0][0].cpu())
    ...
    def process_video(self, video_path: str, prompt: str) -> List[Dict[str, Any]]:
        """Process video frame by frame and return matching segments"""
...
async def search_video(file_id: str, prompt: str):
    try:
        ...
        # Process video
        segments = video_processor.process_video(str(file_path), prompt)
